...
def get_audio_metadata(file_path: Path) -> Tuple[float, str]:
    ...
    try:
        ...
        title = parse_ffprobe_output(file_path, 'format_tags=title') or file_path.stem
        if not title:  # Attempt fallback for common tag names
...
def collect_audio_files(input_paths: List[str]) -> List[Path]:
    ...
    for input_path in input_paths:
        ...
        if path.is_file() and path.suffix.lower() in AUDIO_EXTENSIONS:
            ...
            audio_files.extend(file_path for file_path in path.rglob('*') 
...
Examples:
  ...
  %(prog)s -i file1.mp3 file2.wav -o chapters.txt
    ...
    output_path = (Path(args.output) if args.output else
                   Path.cwd() / (Path(args.input[0]).name if Path(args.input[0]).is_dir() else Path(args.input[0]).stem)) / "_tracklist.txt")
    # Generate tracklist
    if args.verbose:
